...
///
/// 短周期（3天）的小活动按槽位轮换：
/// - 国服：酒馆 → 王牌 → 王牌 → 24h锦标赛（循环）
/// - 国际服：王牌 → 王牌 → 24h锦标赛 → 酒馆 → 太空（循环）
/// 注意：王牌为持续 10 天的活动，占 两个小活动周期 + 中间 1 个大活动周期
/// （3 + 4 + 3 = 10 天）；王牌定位为小活动（图标左上角），
/// 中间那个大活动周期的大活动照常进行，日历上同时显示：右上角正常大活动、
/// 左上角王牌（王牌在大活动进行期间继续存在）。
///
/// 锚点：2026-08-20（周四）19:00 所在活动周期为 weekIndex 0，
/// 该周期 4 天时段：国服 = GP，国际服 = 废铁（下周齿轮）。
/// 短周期锚点：2026-08-24（周一）19:00 所在短周期为 shortIndex 0，
/// 该短周期：国服 = 酒馆，国际服 = 王牌（国际服上个短周期为太空）。
class ActivityCalendar {
  ...
  /// 王牌(占2槽) → 24h锦标赛 → 酒馆 → 太空 → 王牌 → ...
  /// shortIndex 0 = 王牌（锚点短周期），shortIndex -1 = 太空
